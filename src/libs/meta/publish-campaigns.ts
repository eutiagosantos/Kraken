...
import {
  finalizeUploadJob,
  startUploadJobProcessing,
  updateUploadJobDone,
  updateUploadJobProgress,
} from "@/libs/database/queries/upload-jobs";
import { insertActivityEvent } from "@/libs/database/queries/activity-events";
import { insertCampanha, insertCampanhaReturningId } from "@/libs/database/queries/campanhas";

import { uploadAdImageToAccount } from "@/libs/meta/graph-ad-images";
import {
  fetchPreferredAdVideoThumbnail,
  uploadAdVideoChunked,
  waitForAdVideoReady,
} from "@/libs/meta/graph-ad-videos";
import {
  graphCreateAd,
...
} from "@/libs/meta/graph-campaign-publish";
import {
  applyInterestReplacementsToTargeting,
  parseDeprecatedInterestReplacements,
  replacementsToMap,
} from "@/libs/meta/deprecated-targeting-interests";
import {
...
import type { GraphFetch } from "@/libs/meta/graph-client";
...
} from "@/libs/meta/meta-new-account-config";
import {
  buildAdsetSchedulePayload,
  buildFrequencyControlSpecs,
  resolveDailyAdsetFlightForPublish,
  resolveLifetimeScheduleForPublish,
} from "@/libs/meta/campaign-schedule";
import { metaPublishConcurrency, runWithConcurrency } from "@/libs/meta/publish-concurrency";
import type { MetaAppCredentials } from "@/libs/meta/resolve-app-credentials";
import { resolveMetaAppSecret } from "@/libs/meta/resolve-app-credentials";
import { buildUploadJobSummary } from "@/libs/meta/upload-job-summary";
...
async function patchUploadJobProgress(
  ...
  await updateUploadJobProgress(ctx.userId, publishId, done, summary, phase);
}
...
export async function runWizardPublish(ctx: WizardPublishContext): Promise<{
  ...
  const jobRow = await startUploadJobProcessing(ctx.userId, ctx.existingPublishJobId, {
  ...
  if (!jobRow) {
    throw new Error("Falha ao preparar upload_jobs (operação inválida ou já usada).");
  }
  ...
  for (const unit of units) {
    if (unit.fused) {
      ...
      if (missing.length > 0) {
        ...
        await updateUploadJobDone(ctx.userId, publishId, done);
        continue;
      }
      ...
      try {
        ...
        const inserted = await insertCampanhaReturningId({
          user_id: ctx.userId,
  ...
  await finalizeUploadJob(ctx.userId, publishId, {
    status: jobStatus,
